...
(function() {
  ...
  this.toggable = function(parent) {
    return parent.find('.show-logs').on('click', function() {
      return $(this).parents('.subtask:first').find('.subtask-logs').toggle('slow');
    });
  };

  $(function() {
    var i, len, ref, results, subtask;
    ref = $('.subtask');
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      subtask = ref[i];
      results.push(toggable($(subtask)));
    }
    return results;
  });
